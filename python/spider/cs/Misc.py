...
def downloadImg(imgUrl,site):
    newImageUrl=imgUrl
    if 'dnl'==site:
        accountFile='../PostWebAccount/account_config_dnl.txt'
        ...
        if response:
            ...
            webImageDir='/var/www/wordpress/wp-content/uploads/'+year+'/'+month+'/'
            shutil.copy(compressedImage,webImageDir+fileName+'.jpg')
            newImageUrl=webUrl+'/wp-content/uploads/'+year+'/'+month+'/'+fileName+'.jpg'
    print(newImageUrl)
    return (newImageUrl)
...
def push2Baidu(articleId):      
    (baidu_api_url,my_site_url)=getPushBaiduApiConfig('../PostWebAccount/account_config_dnl.txt')
    (year,month)=getDate() 
    push_url=my_site_url+'/articles/'+year+'/'+month+'/'+str(articleId)+'.html'
    print(push_url)
    print(baidu_api_url)
